...
def load_data(name):
    ...
    #all_files = glob.glob("/Users/dineshk/work/MLOps/music/Music-Recommend-Spotify_2/csv_data/*.csv")
    ...
    filtered_df = df[df['artists_name'] == name]
...
def n_neighbors_uri_audio(exploded_track_df, filtered_df, artist_select, genre, start_year, end_year, test_feat):
    ...
    genre_new = find_highest_duplicate(filtered_df.genres)
    #genre_new = str(genre_new)[2:-2]
    #get the genre string value
    genre_new = get_artist_genre(genre_new)
    print('The genre of the given artist: ', genre_new)
    ...
    genre_data = exploded_track_df[(exploded_track_df["genres"]==genre_new) & (exploded_track_df["release_year"]>=min(filtered_df.release_year)) & (exploded_track_df["release_year"]<=max(filtered_df.release_year))]
